...
cache(is_cache_enabled, out_type, funccall) = begin
	...
	esc(quote
			key = (($(args...),),($(kwargs...),))
			!isdefined(Main,$cache_funcname) && (Main.$cache_funcname=Dict{typeof(key),$out_type}())
			if $is_cache_enabled && key in keys(Main.$cache_funcname) 
				Main.$cache_funcname[key]
			else
				Main.$cache_funcname[key] = $funccall
			end
	end)
end
